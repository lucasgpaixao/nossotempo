...
export async function POST(req: Request) {
  try {
    ...
    const { preferenceId, initPoint } = await createCheckoutPreference({
      ...
      kind: "downsell",
      title: `Nosso Tempo — Carta PDF (${o.name1} & ${o.name2})`,
    ...
    await supabaseAdmin()
      .from("orders")
      .update({
        status: "downsell_offered",
        mp_preference_downsell_id: preferenceId,
    ...
    console.error("checkout/downsell", e);
